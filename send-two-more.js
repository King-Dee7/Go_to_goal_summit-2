...
const guests = [
  { name: 'Yvette', email: 'yvettedanquaho5@gmail.com' },
  { name: 'Kofi', email: 'neymarbortey@gmail.com' }
];
...
<body style="margin: 0; padding: 0; background-color: #ffffff;">
  ...
  <div style="background-color: #050505; color: white; padding: 35px; border-radius: 16px; margin: 35px 0; text-align: center;">
    ...
    <p style="font-size: 14px; opacity: 0.8; margin-bottom: 0;">Your name has been added to our guest list at the venue. <strong>Please have this confirmation email ready to show at the door for entry approval.</strong></p>
  </div>
...
async function main() {
  console.log('Starting to send approval emails...');
  ...
  for (const guest of guests) {
    try {
      console.log(`Sending to ${guest.name} (${guest.email})...`);
      ...
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          to: guest.email,
          subject: "You're Officially Invited!",
          html: getEmailHtml(guest.name),
        })
      });
