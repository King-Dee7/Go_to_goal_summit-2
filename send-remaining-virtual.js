...
const innerContent = `
  ...
  <p>Having you join us <strong>virtually</strong> meant the world to us. Even through a screen, your energy and presence as we shared those transformative moments really brought the summit to life in a way we could only dream of. Thank you for showing up, not just for us, but for yourself and your own goals.</p>
...
async function sendRemainingVirtualEmails() {
  ...
  console.log("Found " + trueVirtuals.length + " total true virtual attendees.");
  
  // Skip the first 52 that were sent yesterday, take the rest.
  const batchToSend = trueVirtuals.slice(52);
  
  console.log("Preparing to send " + batchToSend.length + " emails today...");

  let successCount = 0;
  let failCount = 0;

  for (const person of batchToSend) {
    const personalizedInnerContent = innerContent.replace(/{First Name}/g, person.first_name || 'there');
  ...
  console.log("Finished sending branded virtual emails. Success: " + successCount + ", Failed: " + failCount);
}

sendRemainingVirtualEmails();
